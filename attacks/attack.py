...
from attacks.advertorch_utils import clamp
from attacks.advertorch_utils import normalize_by_pnorm
from attacks.advertorch_utils import clamp_by_pnorm
from attacks.advertorch_utils import is_float_or_torch_tensor
from attacks.advertorch_utils import batch_multiply
from attacks.advertorch_utils import batch_clamp
from attacks.advertorch_utils import replicate_input
from attacks.advertorch_utils import batch_l1_proj
...
def perturb_iterative(xvar, yvar, predict, nb_iter, eps, eps_iter, loss_fn,
    ...
    for ii in range(nb_iter):
        ...
        if ord == np.inf:
            ...
            grad = normalize_by_pnorm(grad, p=ord)
...
class PGDAttackOrig(Attack, LabelMixin):
    """
    The projected gradient descent attack (Madry et al, 2017).
    The attack performs nb_iter steps of size eps_iter, while always staying
    within eps from the initial point.
...
def perturb_iterative_orig(xvar, yvar, predict, nb_iter, eps, eps_iter, loss_fn,
                      delta_init=None, minimize=False, ord=np.inf,
                      clip_min=0.0, clip_max=1.0,
                      l1_sparsity=None):
    """
